  ...
  return (
    <div className={`flex items-center justify-center w-full transition-all duration-300 ${isOpen ? "absolute top-0 left-0 w-full bg-white h-24 z-50" : "w-auto"} `} >
        {isOpen ? (<form  onSubmit={handleSearch} className='relative flex items-center justify-center w-full'>
          ...
          <button type='submit' className='absolute right-2 top-1/3 transform ~translate-y-1/2 text-gray-600 hover:text-gray-800'>
          <HiMagnifyingGlass  className="h-6 w-6" />
          </button>
          </div>
          <button type='button' onClick={handleSearchToggle} className='absolute right-2 top-1/3 transform ~translate-y-1/2 text-gray-600 hover:text-gray-800'>
